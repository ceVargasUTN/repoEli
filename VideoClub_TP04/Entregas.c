...
{
    eEntrega* auxRetorno = NULL;
    auxRetorno=(eEntrega*)malloc(sizeof(eEntrega));//reservo  memoria

    strcpy(auxRetorno->cliente_name,cliente);
    strcpy(auxRetorno->tipo_urgencia,urgencia);
    strcpy(auxRetorno->direccion,direccion);
...
eEntrega* entregas_alta()
{
    ...
    char urgencia[50];
    char direccion[50];
    eEntrega* pListEntrega;

    printf("Ingrese el su  nombre: ");
    fflush(stdin);
    gets(cliente);
    printf("Ingrese el tipo de urgencia: ");
    fflush(stdin);
    gets(urgencia);
    ...
    pListEntrega = entregas_new(cliente,urgencia,direccion);
    pListEntrega->flag_estado = OCUPADO;
